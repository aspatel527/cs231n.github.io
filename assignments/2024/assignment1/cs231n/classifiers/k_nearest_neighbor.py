...
class KNearestNeighbor(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                dists[i][j] =  np.sqrt(np.sum((self.X_train[j] - X[i])**2)) # np.sqrt(np.sum(np.square(self.X_train[j] - X[i]), axis = 1))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            y_pred[i] = np.bincount(closest_y).argmax()  # np.bincount(closest_y).argmax()
